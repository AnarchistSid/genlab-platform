"""Blackbox Brief pipeline runner — delegates to unified CLI.
...
def main() -> int:
    parser = build_parser()
    parser.prog = "python run_pipeline.py (BlackboxBrief / ai_creators)"
    # Override --niche to default to ai_creators and not be required
    for action in parser._actions:
        if hasattr(action, "dest") and action.dest == "niche":
            action.required = False
            action.default = "ai_creators"
            break
