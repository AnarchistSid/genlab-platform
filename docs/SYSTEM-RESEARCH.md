...
| R-31 | HIGH | Partly-corrected | **Affiliate networks are mostly stubs.** EarnKaro/Impact/ShareASale/CJ raise `NotImplementedError` (`network_registry.py:~102–138`); PA-API signing is a stub with empty keys → dynamic matching gated off; Cuelinks wrapping strips the Amazon tag → 0 commission. Only **direct Amazon Associates** links work today. Matching is weak (1-keyword hit + evergreen fallback). | sub-item **c** (matching): static-catalog match threshold raised from `>0` to `>=2` keyword hits (mirrors the seasonal threshold); 1-hit candidates fall through to the calibrated evergreen path with a visibility log. sub-item **b** (Cuelinks tag-strip): all 33 Cuelinks URLs in `affiliate_catalog.example.yaml` now carry `?tag=${AMAZON_IN_AFFILIATE_TAG}` inside the embedded Amazon URL; regression pin scans the YAML for any link wrapping Amazon without `tag=`. sub-item **a — PA-API signing done** (`6b2f97b`): the deepest stub in R-31 (a) — `PaapiClient._signed_request` raising `NotImplementedError("AWS Sig v4")` — is closed. Pure-stdlib AWS Signature Version 4 implementation shipped at `genlab-core/src/genlab_core/monetization/paapi_signing.py` (~150 LOC, no boto3); verified against AWS's official `aws4_testsuite` published test vectors (canonical request + string-to-sign + end-to-end signature `5fa00fa31553b73e...` all match what every AWS SDK produces). `_signed_request` now wires through `sign_request_v4` + `requests.post`; `is_configured` gate + graceful-degradation try/except preserved; HTTP errors propagate so `search`/`get_by_asin` can log+degrade. 17 signing pins (4 AWS vectors + 6 PA-API-shape + 3 misuse + 4 stage-helper) + 7 wiring pins. After this PR, only setting `PAAPI_ACCESS_KEY` + `PAAPI_SECRET_KEY` stands between the pipeline and live PA-API queries. sub-item **a — 4 per-network adapters** (EarnKaro / Impact / ShareASale / CJ) **still deferred** — each needs operator credentials AND per-network real-world setup (Impact campaign IDs, ShareASale merchant relationships, CJ PID/AID) to be meaningfully testable; named blocker, not a scope-of-work blocker _(landed: `c49a6b1` + `a7bd949` + `6b2f97b`)_ |
...
| R-70 | MEDIUM | Done | **Missing strategy base classes → ~750L duplication + 826L dead gaming legacy.** No `BaseVisualRenderStrategy`/`BaseScoringStrategy` in `genlab_core/strategies/` (only Writing/Hooks/ContentResearch/PlatformAdaptation) → `visual_render.py` (~258/248/270L in SR/CW/FD) + the per-channel scoring strategies reimplement an identical method set (~750L copy-paste), contradicting the "~50 lines of niche overrides" claim. Gaming's `write_gaming_content_legacy.py` (535L) + `adapt_gaming_content_legacy.py` (291L) remain in-tree, referenced only by `tests/_legacy/` (extends R-20). | **Part 1 done** (`27dc509`): 826 LOC of gaming legacy strategies deleted (R-20's `506d67e` claimed they were gone but they weren't — this commit makes that claim honest); workspace-wide regex pin on import statements catches re-introduction. **Part 2 design phase done** (`dd4c73b`): empirical measurement found audit overstated — actual divergence is 55% (visual_render) and 70% (scoring), parallel-evolved not copy-paste. `docs/r70-part2-design-phase.md` sequences 5-PR extraction with measured-divergence baseline, method inventory per channel, explicit non-decisions deferred to pilot. **Part 2 PR 1 done** (`fd1ef6c`): `BaseVisualRenderStrategy(VisualRenderStrategy)` shipped to `genlab_core/strategies/base_visual_render.py` — one concrete method (`_get_whisper_config`, verified byte-identical across SR/CW/FD before extraction) + 6 abstract for the pilot to fill in. 7 pins cover abstract-contract integrity + `_get_whisper_config` semantics + `isinstance(x, VisualRenderStrategy)` preservation. **Part 2 PR 2 done** (`068006e`): SpliceReel pilot migrated — `MovieVisualRenderStrategy` now inherits from `BaseVisualRenderStrategy`, `__init__` calls `super().__init__()`, the duplicated `_get_whisper_config` body is deleted (now inherited). Design plan's gate met: **134/134 SR tests pass unchanged**. SR's `_ensure_config` stays per-channel because it uniquely reads BOTH `sources.yaml` + `visuals.yaml` (CW + FD only read `visuals.yaml`). **Part 2 PR 3 done** (`4d99393`): ClutchWire + FrameDrift both migrated following the SR pilot pattern — `SportVisualRenderStrategy` and `AnimeVisualRenderStrategy` now inherit from `BaseVisualRenderStrategy`, duplicated `_get_whisper_config` bodies deleted (now inherited). Each channel's `_ensure_config` stays per-subclass: CW reads only `visuals.yaml`; FD reads both `sources.yaml` + `visuals.yaml` (same shape as SR). 23/23 CW + 19/19 FD visual_render tests pass; full channel suites green via CI with the cached BERT from PR #144. **Part 2 PR 4 done** (`36dd8de`): `_compose_frame` extracted into the base — precondition held (bodies byte-identical except for the `[movies]`/`[sports]`/`[anime]` log prefix); base now concrete, reads subclass-set `_log_prefix` + `_visuals_yaml_path` instance attributes; SR/CW/FD each drop ~30L of duplicated body + the now-unused `FrameCompositor` import. 6 new base-level pins on the lifted body (empty `run_dir` guard, FrameCompositor wiring, 60s cap, 55s probe-failure fallback, hook→title fallback, exception → `""` with channel-tagged log). 134 SR + 136 CW + 143 FD full channel suites green; 28/28 base strategy tests green. **Part 2 PR 5a done** (`9078eb3`): `BaseScoringStrategy(ScoringStrategy)` skeleton shipped — empirical body-diff before extraction narrowed the scope from the design doc's "magnitude + novelty" hypothesis to "novelty only" (the only byte-identical 1-line body across SR/CW/FD; `_score_magnitude` is parallel-evolved with the same name but completely different bodies/fields/config keys). 6 pins on contract integrity + novelty semantics + ScoringStrategy isinstance preservation. **Part 2 PR 5b done** (`7a7513f`): `BaseTimeBasedScoringStrategy(BaseScoringStrategy)` second-tier base shipped (SR + FD only — CW genuinely uses a different YAML shape) lifting `__init__` / `_ensure_config` / `_score_timeliness` with 2-attribute escape hatch (`_log_prefix` class attr + `_scoring_yaml_path` instance attr + `_default_timeliness_half_life_hours` class attr for the only literal divergence: SR 48h vs FD 24h). SR pilot migration in same PR; 11 pins on the second-tier contract including cache, config-path resolution, half-life fallback, and `_score_novelty` inheritance chain through MRO. **Part 2 PR 5c done** (`a0df3a0`): final migration — FD scoring moved to the second-tier base; CW moved to the narrow base (NOT the second-tier — sports's `recency`/`community_signal` axes + `clip_scoring` YAML sub-key are CW-specific). 47 FD scoring + 32 CW scoring tests pass; 142 FD + 135 CW full channel suites green on CI. **R-70 part 2 sequence complete — all 7 PRs landed in one session sweep.** _(landed: `27dc509` + `dd4c73b` + `fd1ef6c` + `068006e` + `4d99393` + `36dd8de` + `9078eb3` + `7a7513f` + `a0df3a0`)_ |
