...
CASES = [
    ...
    ("billing_error_terse", lambda: _err(ex.BadRequestError, 400, "billing_error", "Billing error"), True),
    ("billing_error_classic", lambda: _err(
        ex.BadRequestError, 400, "invalid_request_error",
        "Your credit balance is too low to access the Anthropic API."), True),
    ("insufficient_credits", lambda: _err(
        ex.BadRequestError, 400, "invalid_request_error", "insufficient credits"), True),
    # TRANSIENT — must NOT fail over; retry on the primary.
    ("rate_limit_429", lambda: _err(ex.RateLimitError, 429, "rate_limit_error", "rate limited"), False),
    ("overloaded_529", lambda: _err(ex.OverloadedError, 529, "overloaded_error", "Overloaded"), False),
    ("internal_500", lambda: _err(ex.InternalServerError, 500, "api_error", "Internal server error"), False),
    ("service_unavailable_503", lambda: _err(
        ex.ServiceUnavailableError, 503, "api_error", "unavailable"), False),
    # OPERATOR PROBLEM — not should_fallback's business.
    ("auth_401", lambda: _err(ex.AuthenticationError, 401, "authentication_error", "invalid x-api-key"), False),
]
