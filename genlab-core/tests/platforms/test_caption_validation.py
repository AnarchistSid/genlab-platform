...
def test_validate_accepts_ai_generated_marker():
    """2026-08-18 (task #192): AI-generated backfill content (from
    pruna_video_client.generate_backfill_clip) must satisfy L4 with
    the honest '🤖 AI-generated' marker instead of dressing it up as
    external creator credit."""
    from genlab_core.platforms.caption_validation import (
        validate_caption_has_attribution,
    )

    cap = "Anime moment inspired by trending topic.\n\n\U0001f916 AI-generated by Gen Lab"
    ok, reason = validate_caption_has_attribution(cap)
    assert ok is True
    assert reason is None


def test_validate_rejects_ai_marker_with_empty_tail():
    """Same tail-length rule as other markers — bare marker without
    a following identifier is rejected."""
    from genlab_core.platforms.caption_validation import (
        validate_caption_has_attribution,
    )

    cap = "Body.\n\n\U0001f916 AI-generated"  # no trailing content
    ok, reason = validate_caption_has_attribution(cap)
    assert ok is False
