"""SpliceReel pipeline runner — delegates to unified CLI.
...
def main() -> int:
    parser = build_parser()
    parser.prog = "python run_pipeline.py (SpliceReel / movies)"
    # Override --niche to default to movies and not be required
    for action in parser._actions:
        if hasattr(action, "dest") and action.dest == "niche":
            action.required = False
            action.default = "movies"
            break
