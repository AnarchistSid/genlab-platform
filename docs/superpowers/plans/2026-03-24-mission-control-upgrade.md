...
**Backend test:** `curl -s -u "admin:***REMOVED***" http://localhost:5151/api/v1/<endpoint> | python3 -m json.tool`
...
sleep 3
curl -s -u "admin:***REMOVED***" http://localhost:5151/api/v1/cross-niche/overview | python3 -c "
import json, sys
...
- [ ] **Step 3:** Verify:
```bash
curl -s -u "admin:***REMOVED***" http://localhost:5151/api/v1/analytics/top-posts | python3 -c "
import json, sys
d = json.load(sys.stdin).get('data', {})
...
- [ ] **Step 3:** Verify:
```bash
curl -s -u "admin:***REMOVED***" http://localhost:5151/api/v1/pipeline/quality-stats | python3 -m json.tool
```
