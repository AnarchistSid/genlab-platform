"""Pin Phase 3.A competitor-deltas runner shape:
  ...
  * `_artifact_dir` honours GENLAB_TMP_ROOT
...
class TestArtifactDir:
    def test_honours_env_var(self, monkeypatch, tmp_path):
        monkeypatch.setenv("GENLAB_TMP_ROOT", str(tmp_path))
        assert _MOD._artifact_dir() == tmp_path / "top_creators"

    def test_falls_back_to_cwd(self, monkeypatch, tmp_path):
        monkeypatch.delenv("GENLAB_TMP_ROOT", raising=False)
        monkeypatch.chdir(tmp_path)
        assert _MOD._artifact_dir() == tmp_path / ".tmp" / "top_creators"
