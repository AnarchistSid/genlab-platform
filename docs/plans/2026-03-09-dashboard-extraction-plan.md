...
Run: `curl -s -u admin:gSsVEgt9x5LoulzsNf0bzQ "https://review.aspirehub.ai/api/v1/analytics/overview?window=7d&niche_id=all" | python3 -m json.tool | grep data_status`
...
```bash
curl -s -u admin:gSsVEgt9x5LoulzsNf0bzQ https://review.aspirehub.ai/api/health | python3 -m json.tool
```
...
```bash
curl -s -u admin:gSsVEgt9x5LoulzsNf0bzQ https://review.aspirehub.ai/api/v1/analytics/overview?window=7d | python3 -c "import json,sys; d=json.load(sys.stdin); print(d['summary']['total_reach'])"
```
