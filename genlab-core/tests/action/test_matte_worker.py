...
REQ = MatteRequest(clip_path="/clips/a.mp4", frames_dir="/frames/a",
                   niche_id="sports", blueprint_id="bp1")
...
def test_a_timeout_returns_a_reason_and_never_hangs(q):
    ...
    res, reason = request_matte(
        REQ, root=q, timeout_s=60, poll_s=5,
...
def test_a_completed_job_returns_the_mask_dir(q):
    ...
    res, reason = request_matte(REQ, root=q, timeout_s=600, poll_s=5,
...
def test_the_job_file_appears_atomically(q):
    ...
    def sleep(s):
        ...
        for p in (q / "queued").glob("*.json"):
            jid = json.loads(p.read_text())["job_id"]
            (q / "done" / f"{jid}.json").write_text(
                json.dumps({"mask_dir": "/m", "frames": 1}))

    clock = {"t": 0.0}
    request_matte(REQ, root=q, timeout_s=60, poll_s=1, sleep=sleep,
...
def test_every_skip_reason_is_a_distinct_string():
    vals = [SkipReason.WORKER_UNAVAILABLE, SkipReason.TIMEOUT,
