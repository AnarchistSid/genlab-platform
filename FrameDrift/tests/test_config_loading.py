"""Tests for FrameDrift config loading."""
...
class TestConfigLoading:
    ...
    def test_niche_yaml_required_keys(self):
        ...
        assert data["niche_id"] == "anime"
        assert data["display_name"] == "FrameDrift"
        assert data["accent_color"] == "#7B3FE4"
        assert "brand_voice" in data
        assert "feature_flags" in data

    def test_sources_has_rss_feeds(self):
        data = _load("sources.yaml")
        tier_1_types = [s["type"] for s in data["tier_1"]["sources"]]
        assert "rss" in tier_1_types
    ...
    def test_trend_cycle_multipliers_present(self):
        data = _load("scoring_weights.yaml")
        mults = data["trend_cycle_multipliers"]
        assert "emerging" in mults
        assert "declining" in mults
        assert mults["emerging"] > mults["declining"]
