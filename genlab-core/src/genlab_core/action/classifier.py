...
def classify(sig: Signals) -> Verdict:
    ...
    if sig.motion_energy < STILL_MOTION_MAX:
        return Verdict("STILL", _conf(STILL_MOTION_MAX - sig.motion_energy, 2.0),
                       f"motion {sig.motion_energy:.2f} < {STILL_MOTION_MAX} — "
                       f"nothing to cut to", sig)

    if sig.speech_ratio >= TALK_SPEECH_MIN and sig.face_persistence >= TALK_FACE_MIN:
        return Verdict("TALK", _conf(min(sig.speech_ratio - TALK_SPEECH_MIN,
                                         sig.face_persistence - TALK_FACE_MIN), 0.3),
                       f"speech {sig.speech_ratio:.2f} and a held face "
                       f"{sig.face_persistence:.2f}", sig)

    if sig.motion_energy >= ACTION_MOTION_MIN and sig.face_persistence <= ACTION_FACE_MAX:
        return Verdict("ACTION", _conf(sig.motion_energy - ACTION_MOTION_MIN, 8.0),
                       f"motion {sig.motion_energy:.2f} with no held face "
                       f"({sig.face_persistence:.2f})", sig)
    ...
    return Verdict("STILL", 0.35,
                   f"ambiguous: motion {sig.motion_energy:.2f}, speech "
                   f"{sig.speech_ratio:.2f}, face {sig.face_persistence:.2f}", sig)
...
def measure(path: str, *, speech_fn: Callable[[str], float],
            motion_fn: Callable[[str], float],
            face_fn: Callable[[str], float]) -> Signals:
    """Signal extraction, injected so this module stays free of ffmpeg and cv2."""
    return Signals(speech_ratio=float(speech_fn(path)),
...
def classify_clip(path: str, **fns) -> Verdict:
    ...
    logger.info("[classifier] %s -> %s (%.2f): %s", path, v.treatment,
